...
int CVICII::update(long cycleCount) {
    ...
    if (_regRASTER == _rasterIrqLine) {
        ...
        */
        // @fixme THIS IS A BUG, the correct code is the one commented above. it
        // seems working this way, but it's wrong (there's something wrong which
        // causes the cia1 timer do not tick, so the irq is not generated by
        // cia1 and generating this here 'seems' to make everything working
...
void CVICII::read(uint16_t address, uint8_t *bt) {
    ...
    case 0xd01a:
        ...
        *bt = (_regInterruptEnabled | 0xf0);
        // SDL_Log("reading intenable=%x", *bt);
        break;
...
void CVICII::write(uint16_t address, uint8_t bt) {
    ...
    case 0xd019:
        // interrupt latch
        _regInterrupt = bt;
        // SDL_Log("setting intlatch=%x", bt);
        break;

    case 0xd01a:
        // interrupt enabled
        _regInterruptEnabled = bt;
        // SDL_Log("setting intenable=%x", bt);
        break;
