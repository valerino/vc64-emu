...
/**
 * a callback for memory writes, allows client to replace the value being
 * written and/or possibly write at another address.
 */
 ...
 * and/or possibly read at another address.
 */
...
class CMOS65xx {
  private:
    // each opcode handler takes the addressing mode and number of cycles for
    // that addressing mode in input, and returns the number of cycles
    // effectively occupied by the instruction (which may be different than the
    // input cycles) and the instruction size
    typedef void (CMOS65xx::*OpcodePtr)(int opcodeByte, int addressingMode,
                                        int *cycles, int *size);
    ...
    uint16_t popWord();
    uint8_t popByte();
    CpuCallbackRead _callbackR = nullptr;
    CpuCallbackWrite _callbackW = nullptr;
    ...
    void logExecution(const char *opcodeName, uint8_t opcodeByte,
                      uint16_t operand, int addressingMode);
    int parseInstruction(uint8_t opcodeByte, const char *functionName,
    ...
    bool _isGoDbgCommand = false;
    bool _mustBreakAfter = false;
    uint32_t _bpAddress = 0;
    int64_t _bpCycles = 0;
    bool _bpSet = false;
    uint64_t _currentTotalCycles;
    bool _silenceLog = false;
    bool _breakIrq = false;
    bool _breakNmi = false;
    bool _breakIrqOccured = false;
    bool _breakNmiOccurred = false;
    bool _ignoreMapping = false;

  public:
    /**
     * run the cpu for an instruction
     * @param debugging if true, debugging prompt is shown after executing
     * @param mustBreakAfter if true, must break after executing this step (only
     * if debugging is true also)
     * @return instruction cycles, or -1 to stop
     */
    int step(bool debugging = false, bool mustBreakAfter = false);

    /**
     * resets the cpu and initializes for a new run
     * @param loadTests if true, test mode is activated and
     * <currentdir>/tests/6502_functional_test.bin is loaded
     ...
     */
    CMOS65xx(IMemory *mem, CpuCallbackRead callbackRead,
