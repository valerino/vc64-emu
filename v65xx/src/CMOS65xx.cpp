    ...
    if (doPrint && !_silenceLog) {
        ...
        if (addressingMode == ADDRESSING_MODE_IMPLIED) {
            ...
            CLog::printRaw("\t$%04x: %02X %02X %02X\t\t%s $%04x\t\t%s\n",
                           _regPC, opcodeByte, opByte2, opByte1, name, operand,
                           statusString);
            ...
            CLog::printRaw("\t$%04x: %02X %02X\t\t%s ($%02x, X)\t\t%s\n",
                           _regPC, opcodeByte, operand, name, operand,
                           statusString);
...
void CMOS65xx::debugger(int addressingMode, int *size) {
    ...
    if (doBreak) {
        ...
        case 'f':
            ...
            if (_ignoreMapping) {
                CLog::printRaw(
                    "'d' and 'e' set to use client memory mapping\n");
            } else {
        ...
        case 'h': {
            ...
            CLog::printRaw("\te <$address> <1a,2b,...>:\twrite given hex bytes "
                           "at address\n");
            CLog::printRaw("\tf:\t\t\t\ttoggle force 'd' and 'e' to use raw "
                           "memory disregarding of mapping\n");
            CLog::printRaw("\tbp <$address>:\t\t\tbreak at address (overwrite "
                           "existent)\n");
            CLog::printRaw("\tbc <num_cycles>:\t\tbreak when the cpu reaches "
                           "(or exceeds) the given cyclecount\n");
